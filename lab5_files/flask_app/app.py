...
db_config = {
    'user': 'root',         # Your MySQL username
    'password': '',         # Your MySQL password
    'host': '13.60.47.185', # MySQL host (e.g., AWS IP address)
    'database': 'visits'    # Your MySQL database name
}
...
# Function to increment the visit count inside the home route
@app.route("/")
def home():
    try:
        # Connect to the MySQL database
        ...
        if result:
            ...
            # If no record exists, initialize the count to 1
            cursor.execute("INSERT INTO visits (id, count) VALUES (1, 1)")
            print("Initialized count to 1.")  # Debugging output

        conn.commit()  # Commit the changes

        # Retrieve the current count after updating/inserting
        cursor.execute("SELECT count FROM visits WHERE id = 1")
        current_count = cursor.fetchone()[0]
        print("Retrieved current count:", current_count)  # Debugging output
    ...
    return f"This page has been visited {current_count} times."
